$(document).ready(function() {
  ...
  // var domain = 'http://www.cleaneats.nyc';
  // var domain = 'http://seamless-health-grades.herokuapp.com';
  var domain = 'http://localhost:3000';

  var gradeNotAvailableLink = "<a href='" + domain + "/health_inspections/'" +
          ...
          $.ajax({
            ...
            success: function(response) {
              ...
              $.each(response.grades_not_found, function(key, val) {
                ...
                $.each($restaurantElements, function(index, restaurantElement) {
                  insertGrade(restaurantElement, domain, 'NotYetGraded', true, true, 'font-size:12px;float:right');
                });
              });
            }
          });
    ...
    $.ajax({
      ...
      success: function(response) {
        ...
        if (response['response'] === 'Grade not found') {
          ...
          showPage    = domain + '/health_inspections/';
        } else {
          letterGrade = standardizeLetterGrade(response.grade);
          showPage    = domain + response.path;
        }
...
function insertGrade(restaurantElement, showPage, letterGrade, showViolationsLink, append, style) {
  ...
  removeSpinner(restaurantElement);
  $(restaurantElement).addClass('grade-added');
