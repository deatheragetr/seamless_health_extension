$(document).ready(function() {
  ...
  if (document.URL.match(/food-delivery\/home/) !== null) {
    $('#HeaderContainer').bind('DOMSubtreeModified', function() {
      ...
      if (currentContainerSize !== containerSize && currentContainerSize > 0) {
        ...
        $.each($('h4.restaurant-name'), function(index, element) {
          var $restaurantLink = $(element).find('a');
        ...
        $.ajax({
          ...
          success: function(response) {
            var gradesFound = response.grades_found,
                gradesNotFound = response.grades_not_found;
                ...
                $(restaurantElement).addClass('grade-added')
                  .append(
                    "<img src='chrome-extension://" + chrome.runtime.id + "/images/Grade" + letterGrade + "_2.jpg'" +
                      "alt='Letter Grade: " + letterGrade + "' style='float:right'/>"
                );
              })
            })

            $.each(gradesNotFound, function(key, val) {
