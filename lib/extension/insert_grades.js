                  ...
                  $restaurantElement.removeClass('grade-searching')
                    ...
                    .append(
                      "<img src='chrome-extension://" + chrome.runtime.id + "/images/Grade" + letterGrade + "_2.jpg'" +
                        "alt='Letter Grade: " + letterGrade + "' style='float:right;height:20px'/>"
                  );
                }
              })
            })
            ...
            $.each(response.grades_found, function(key, val) {
              ...
              if (!$restaurantElement.hasClass('grade-added')) {
                if (letterGrade === 'Not Yet Graded') {
                  insertGrade($restaurantElement, showPage, 'NotYetGraded');
                } else {
                  insertGrade($restaurantElement, showPage, letterGrade);
                }
              }
            });
...
function insertGrade(restaurantElement, showPage, letterGrade) {
  removeSpinner(restaurantElement);

  $(restaurantElement).removeClass('grade-searching')
      .addClass('grade-added')
      .append(
        "<a href='" + showPage + "' target='_blank' style='font-size:12px;float:right'>" +
        "(See Violations) " +
        "<img src='chrome-extension://" + chrome.runtime.id + "/images/Grade" + letterGrade + "_2.jpg'" +
